...
public class NoteTabFragment extends Fragment implements ViewPager.OnPageChangeListener{
	...
	static public NotePageData mNotePageData;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(layout.note_tab, container, false);

		NotePageData.getNoteData();
		if (mNotePageData.mPagelist.size() > 0) {
			setSelectedPageNumber(mNotePageData.mPagelist.get(0).position);
		}

		mSectionsPagerAdapter = new SectionsPagerAdapter(this.getChildFragmentManager());

		mViewPager = (ViewPager) rootView.findViewById(id.note_tab_pager_container);
		mViewPager.setAdapter(mSectionsPagerAdapter);

		topTabIndicator = (MyHorizontalScrollView) rootView.findViewById(id.note_tab_indicator);
	...
	public static class PlaceholderFragment extends Fragment {
		...
		public class MyAdapter extends BaseAdapter {
			...
			private void showImage(int pageNumber, int rowNumber) {
				showImageIntent.putExtra(ShowImageActivity.EXTRA_MESSAGE_IMAGE_NUMBER, mNotePageData.mPagelist.get(pageNumber).rowList.get(rowNumber).dbPrimaryKey);
			...
			public int getCount() {
				...
				return mNotePageData.mPagelist.get(pageNumber).rowList.size();
			}
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				holder.row_name.setText(mNotePageData.mPagelist.get(pageNumber).rowList.get(position).name.toString());
				holder.value_text.setText(mNotePageData.mPagelist.get(pageNumber).rowList.get(position).text.toString());
				holder.image.setImageBitmap(mNotePageData.mPagelist.get(pageNumber).rowList.get(position).image);
				if (mNotePageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_IMAGE) {
				...
				if (mNotePageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_PASSWORD) {
					holder.value_text.setTransformationMethod(PasswordTransformationMethod.getInstance());
	...
	public class SectionsPagerAdapter extends FragmentStatePagerAdapter {
		...
		@Override
		public int getCount() {
			return NotePageData.getNoteTabNumber();
		}
		...
		public CharSequence getPageTitle(int position) {
			return (mNotePageData.mPagelist.get(position).title.toString());
		}
	...
	public void onPageSelected(int position) {
		selectedPageNumber = mNotePageData.mPagelist.get(position).position;
	}
