...
public class PaymentTabFragment extends Fragment implements ViewPager.OnPageChangeListener{
	...
	static public PaymentPageData mPaymentPageData;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(layout.payment_tab, container, false);

		PaymentPageData.getPaymentData();
		if (mPaymentPageData.mPagelist.size() > 0) {
			setSelectedPageNumber(mPaymentPageData.mPagelist.get(0).position);
		}

		mSectionsPagerAdapter = new SectionsPagerAdapter(this.getChildFragmentManager());

		mViewPager = (ViewPager) rootView.findViewById(id.payment_tab_pager_container);
		mViewPager.setAdapter(mSectionsPagerAdapter);

		topTabIndicator = (MyHorizontalScrollView) rootView.findViewById(id.payment_tab_indicator);
	...
	public static class PlaceholderFragment extends Fragment {
		...
		public class MyAdapter extends BaseAdapter {
			...
			private void showImage(int pageNumber, int rowNumber) {
				showImageIntent.putExtra(ShowImageActivity.EXTRA_MESSAGE_IMAGE_NUMBER, mPaymentPageData.mPagelist.get(pageNumber).rowList.get(rowNumber).dbPrimaryKey);
			...
			public int getCount() {
				...
				return mPaymentPageData.mPagelist.get(pageNumber).rowList.size();
			}
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				holder.row_name.setText(mPaymentPageData.mPagelist.get(pageNumber).rowList.get(position).name.toString());
				holder.value_text.setText(mPaymentPageData.mPagelist.get(pageNumber).rowList.get(position).text.toString());
				holder.image.setImageBitmap(mPaymentPageData.mPagelist.get(pageNumber).rowList.get(position).image);
				if (mPaymentPageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_IMAGE) {
				...
				if (mPaymentPageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_PASSWORD) {
					holder.value_text.setTransformationMethod(PasswordTransformationMethod.getInstance());
	...
	public class SectionsPagerAdapter extends FragmentStatePagerAdapter {
		...
		@Override
		public int getCount() {
			return PaymentPageData.getPaymentTabNumber();
		}
		...
		public CharSequence getPageTitle(int position) {
			return (mPaymentPageData.mPagelist.get(position).title.toString());
		}
	...
	public void onPageSelected(int position) {
		selectedPageNumber = mPaymentPageData.mPagelist.get(position).position;
	}
