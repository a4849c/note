...
import static com.may6soft.notepad.R.*;

public class WebsiteTabFragment extends Fragment implements ViewPager.OnPageChangeListener{
	...
	static public WebsitePageData mWebsitePageData;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(layout.website_tab, container, false);

		WebsitePageData.getWebsiteData();
		if (mWebsitePageData.mPagelist.size() > 0) {
			setSelectedPageNumber(mWebsitePageData.mPagelist.get(0).position);
		}

		mSectionsPagerAdapter = new SectionsPagerAdapter(this.getChildFragmentManager());

		mViewPager = (ViewPager) rootView.findViewById(id.website_tab_pager_container);
		mViewPager.setAdapter(mSectionsPagerAdapter);

		topTabIndicator = (MyHorizontalScrollView) rootView.findViewById(id.website_tab_indicator);
	...
	public static class PlaceholderFragment extends Fragment {
		//private WebsitePageData mPageData;
		/**
		...
		public class MyAdapter extends BaseAdapter {
			...
			private void showImage(int pageNumber, int rowNumber) {
				showImageIntent.putExtra(ShowImageActivity.EXTRA_MESSAGE_IMAGE_NUMBER, mWebsitePageData.mPagelist.get(pageNumber).rowList.get(rowNumber).dbPrimaryKey);
			...
			public int getCount() {
				...
				return mWebsitePageData.mPagelist.get(pageNumber).rowList.size();
			}
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				holder.row_name.setText(mWebsitePageData.mPagelist.get(pageNumber).rowList.get(position).name.toString());
				holder.value_text.setText(mWebsitePageData.mPagelist.get(pageNumber).rowList.get(position).text.toString());
				holder.image.setImageBitmap(mWebsitePageData.mPagelist.get(pageNumber).rowList.get(position).image);
				if (mWebsitePageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_IMAGE) {
				...
				if (mWebsitePageData.mPagelist.get(pageNumber).rowList.get(position).rowType == GlobalDataStructure.ROW_PASSWORD) {
					holder.value_text.setTransformationMethod(PasswordTransformationMethod.getInstance());
	...
	public class SectionsPagerAdapter extends FragmentStatePagerAdapter {
		...
		@Override
		public int getCount() {
			return WebsitePageData.getWebsiteTabNumber();
		}
		...
		public CharSequence getPageTitle(int position) {
			return (mWebsitePageData.mPagelist.get(position).title.toString());
		}
	...
	public void onPageSelected(int position) {
		selectedPageNumber = mWebsitePageData.mPagelist.get(position).position;
	}
